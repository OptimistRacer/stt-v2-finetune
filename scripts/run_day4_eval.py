...
def run(config_path: str, split: str, limit: int | None, batch_size: int) -> None:
    ...
    print(f"[day4] evaluating {len(records)} clips from split={split}")
    ...
    base_results = transcribe_records(base_model, processor, records, sample_rate, batch_size)
    ...
    lora_results = transcribe_records(lora_model, processor, records, sample_rate, batch_size)
